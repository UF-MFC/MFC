...
contains
    ...
    subroutine s_interp_fluid_properties(pos, cell, q_prim_vf, wx, wy, wz, fluid_vel, fluid_rho, fluid_pres, alpha, alpha_rho)
        ...
        #:if not MFC_CASE_OPTIMIZATION and USING_AMD
            real(wp), dimension(3), intent(out) :: alpha, alpha_rho
        #:else
        ...
        if (lag_params%interpolation_order > 1) then
            ...
            do l = 1, num_fluids
                alpha_rho(l) = q_prim_vf(l)%sf(cell(1), cell(2), cell(3))
                fluid_rho = fluid_rho + alpha_rho(l)
            end do
            ...
            if (num_fluids == 1) then
                ...
                do l = 1, num_fluids - 1
                    alpha(l) = q_prim_vf(eqn_idx%adv%beg + l - 1)%sf(cell(1), cell(2), cell(3))
                end do
                alpha(num_fluids) = 1._wp - sum(alpha(1:num_fluids - 1))
            end if
    ...
    subroutine s_compute_qs_fluctuations(vel_p, fluid_vel, fluid_rho, cson, gas_mu, particle_diam, vol_frac, vmag, vel_p_mean, &
        ...
        call s_prng(UnifRnd(1), seed)
        call s_prng(UnifRnd(2), seed)
        call s_prng(UnifRnd(3), seed)
        call s_prng(UnifRnd(4), seed)
        call s_prng(UnifRnd(5), seed)
